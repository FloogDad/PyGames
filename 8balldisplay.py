...
Q1 = tk.Label(left_frame, text="What is your question?")
Q1.grid(row=1, column=0)

E1 = tk.Entry(left_frame)
...
def prediction():
    if E1.get() == "" or "?" not in E1.get():
        ...
        tk.Label(T1, text="You haven't asked a question!").pack()
        
    else:
        T1 = tk.Toplevel(window)
        T1.title("Prediction")
        tk.Label(T1,text =random.choice(val)).pack()
...
B1 = tk.Button(left_frame,text="Shake the magic 8-ball!",command=prediction)
